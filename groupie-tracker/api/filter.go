...
func CheckFilterFields(params FilterParams) bool {
	...
	if params.Filter.FirstAlbumDate.IsEnabled {
		_, err := time.Parse(timeLayoutForClient, params.Filter.FirstAlbumDate.FromDate)
		if err != nil {
			return false
		}
		_, err = time.Parse(timeLayoutForClient, params.Filter.FirstAlbumDate.BeforeDate)
		if err != nil {
			return false
		}
	}
	if params.Filter.CreationDate.IsEnabled && (params.Filter.CreationDate.FromYear < 0 || params.Filter.CreationDate.BeforeYear < 0) {
		return false
	}
	if params.Filter.CountMembers.IsEnabled && (params.Filter.CountMembers.From < 0 || params.Filter.CountMembers.Before < 0) {
		return false
	}
	return true
}
...
func searchResult(artist Artists, looking string) bool {
...
func filterResult(artist Artists, filter FilterParams) bool {
